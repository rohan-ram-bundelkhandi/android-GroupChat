<?php
   ...
   /**
     ...
     * returns user details
     */
   function storeUserChat($IMEIUser, $nameUser,$messageUser) 
   {
        // insert user into database
        $result = mysql_query("INSERT INTO users_chat(user_name,user_imei, user_message,created_at) VALUES('$nameUser', '$IMEIUser','$messageUser', NOW())");
        ...
        if ($result) 
	{
            // get user details
            $id = mysql_insert_id(); // last inserted id
            $result = mysql_query("SELECT * FROM users_chat WHERE id = $id") or die(mysql_error());
            // return user details
            if (mysql_num_rows($result) > 0) 
	    {
                return mysql_fetch_array($result);
   ...
   function getAllUsers() 
   {
    ...
    /**
     * Getting all users chat
     */
   function getAllUsersChat($local_db_rows) 
   {
	$local_no_rows=(int) $local_db_rows;
	$server_no_rows=(int)getUserDataCount();  
	
	$diff=((int)$server_no_rows - (int)$local_no_rows);
	
      	$result = mysql_query("select * FROM users_chat order by created_at LIMIT $diff ");

        return $result;
    }

    /**
     * Getting number of rows in server db
     */

    function getUserDataCount()
    {
	 $result = mysql_query("select count(*) as cnt FROM users_chat");
